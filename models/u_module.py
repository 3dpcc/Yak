...
class Enhancer_u(torch.nn.Module):
    def __init__(self):
        super().__init__()
    ...
    def encode(self, x_ori, gt):
        ...
        u = feats[:, 1:2]
        true_u = label[:, 1:2]
        x = ME.SparseTensor(features=u.float(), coordinates=x_ori.C, device=x_ori.device)
        ...
        true_u = true_u.cuda()
        b_feature = torch.subtract(true_u, x.F)
    ...
    def decode(self,x_ori, A):
        ...
        final = residual + x.F
        feats[:,1:2] = final.clone()
        x_out = ME.SparseTensor(features=feats, coordinates=x_ori.C, device=x_ori.device)
        return x_out
